...
class HappyMondaysTest < Test::Unit::TestCase
  ...
  def test_week_start_day
    ...
    assert_equal  1, @my_date.wday
    @my_date.week_start_day = 'tuesday'
    assert_equal  2, @my_date.wday
    @my_date.week_start_day = 'wednesday'
    assert_equal  3, @my_date.wday
    @my_date.week_start_day = 'thursday'
    assert_equal  4, @my_date.wday
    @my_date.week_start_day = 'friday'
    assert_equal  5, @my_date.wday
    @my_date.week_start_day = 'saturday'
    assert_equal  6, @my_date.wday
  end

  def test_orig_wday
    @my_new_date = Date.new(2011,2,8)
    assert_equal 2, @my_new_date.orig_wday
  end
  ...
  def test_nondefault_beginning_of_week
    ...
    @my_date.week_start_day = 'sunday'
    assert_equal  0, @my_date.wday
    assert_equal  'Sun 02/06/2011', @my_date.beginning_of_week.strftime('%a %m/%d/%Y')

    @my_new_date = Date.new(2011, 2, 8)
    @my_new_date.week_start_day = 'Tuesday' # non downcase
    assert_match  'Tuesday', @my_new_date.week_start_day
    assert_equal  'Tue 02/08/2011', @my_new_date.beginning_of_week.strftime('%a %m/%d/%Y')
  end
  ...
  def test_nondefault_end_of_week
    @my_date.week_start_day = 'tuesday'
    assert_equal  'Mon 02/14/2011', @my_date.end_of_week.strftime('%a %m/%d/%Y')
  end
  ...
  def test_nondefault_next_week
    @my_date.week_start_day = 'tuesday'
    assert_equal  'Tue 02/15/2011', @my_date.next_week.strftime('%a %m/%d/%Y')
  end
  ...
  def xtest_using_thread_outside_of_self
    ...
    assert_equal  'Sun 02/06/2011',   @thread_date.beginning_of_week.strftime('%a %m/%d/%Y')
    assert_equal  'Wed 02/09/2011',   @thread_date.end_of_week.strftime('%a %m/%d/%Y')
    assert_equal  'Sun 02/13/2011',   @thread_date.next_week.strftime('%a %m/%d/%Y')
  end
end
